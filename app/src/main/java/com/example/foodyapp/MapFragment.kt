...
import com.example.foodyapp.Util.FoodItem
...
class MapFragment : Fragment() {
    ...
    private lateinit var foodGraphicLayer: GraphicsOverlay
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (GlobalBox.selectedFoodItem != null) {
            foodItemList.add(GlobalBox.selectedFoodItem!!)
            drawHouseDataOnMap()
            zoomToAllExtent()
        } else {
    ...
    private fun fetchDataFromDatabase() {
        ...
        firebaseRepository.getSavedFoodDataFromOnlineDB().get().addOnSuccessListener { documents ->
            ...
            for (document in documents) {
                val houseData = document.toObject(FoodItem::class.java)
                foodItemList.add(houseData)
            }
            filterFoodDataOnlyInBuffer()
            drawHouseDataOnMap()
            zoomToAllExtent()
        }
    }
    ...
    private fun drawHouseDataOnMap() {
        ...
        for (houseData in foodItemList) {
            ...
            foodGraphicLayer.graphics.add(pointGraphic)
        }
    }
    ...
    private fun addLayerGraphicOverlay() {
        ...
        foodGraphicLayer = GraphicsOverlay()
        mapView.graphicsOverlays.add(graphicLayer)
        mapView.graphicsOverlays.add(foodGraphicLayer)
    }
